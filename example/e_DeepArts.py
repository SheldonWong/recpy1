...
from model import dltmf
import collections
import numpy as np 
import pandas as pd
from nlp import deep as ae
...
#4. 计算主题
print("step1:计算主题")
doc_topic_df,item_vector_dict = ae.getDeepFeature(trainset)
...
pref_l = np.random.rand(len(user_l),5).tolist()
...
'''
LSTMF = lstmf.LSTMF(dataname,train_list,testset,m,n,u_dict,v_dict,5,20,0.5,0.01)
u,v,bu,bi,ave= LSTMF.train_by_list()

STMF = stmf.STMF(dataname,train_list,testset,m,n,u_dict,v_dict,5,20,0.5,0.01)
u,v,bu,bi,ave= STMF.train_by_list()
'''
DLTMF = dltmf.DLTMF(dataname,train_list,testset,m,n,u_dict,v_dict,5,30,0.5,0.01)
u,v,bu,bi,ave= DLTMF.train_by_list()