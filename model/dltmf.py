    ...
    def train_by_list(self):
        ...
        for t in range(self.t):
            ...
            for row in self.a:
                ...
                score = self.global_bias  + self.bu[i] + self.bi[j] + np.dot(p,self.wv[j]*q)

                sig_score = 1.0 / (1+math.exp(-score))
                rating_hat = self.min_rating + sig_score * (self.max_rating - self.min_rating)
                err = row[1] - rating_hat
                for r in range(self.k):
                    gu = err * sig_score * sig_score * (1 - sig_score) * self.wv[j] * q[r] - self.lamda * self.u[i][r]
                    self.u[i][r] += self.alpha * gu
                
                gv = err * sig_score * sig_score * (1 - sig_score)  * np.dot(p,q) - self.lamda * self.wv[j] 
        ...
        return self.u,self.wv,self.bu,self.bi,self.global_bias 
